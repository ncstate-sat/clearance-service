...
class ClearanceAssignment:
    ...
    def get_clearances_by_assignee(assignee_id: str) -> list["Clearance"]:
        ...
        return [
            Clearance(clearance.get("GUID"), clearance.get("ObjectID"), clearance.get("Name"))
            for clearance in assigned_clearances
        ]

    @classmethod
    def get_assignments_by_assignee(cls, assignee_id: str) -> list["ClearanceAssignment"]:
        """
        ...
        return [
            ClearanceAssignment(clearance_id=clearance.id, clearance_name=clearance.name)
    ...
    def assign(
        ...
        if start_time or end_time:  # then add it to mongo
            ...
            assignment_collection = get_clearance_collection("clearance_assignment")
    ...
    @staticmethod
    def revoke(assigner_email: str, assignee_ids: list[str], clearance_ids: list[str]) -> int:
        """
