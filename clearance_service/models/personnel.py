...
class Personnel:
    ...
    def __init__(
        self, first_name=None, middle_name=None, last_name=None, email=None, campus_id=None
    ):
    ...
    def clearances(self) -> list[str]:
        ...
        clearances = ClearanceAssignment.get_clearances_by_assignee(self.campus_id)
    ...
    def assign(
        ...
        return ClearanceAssignment.assign(
            assigner_email, [self.campus_id], clearances, start_time, end_time
        )
    ...
    def revoke(self, assigner_id: str, clearances: list[str]) -> int:
        ...
        Returns: the number of changes made
        """
        return ClearanceAssignment.revoke(assigner_id, [self.campus_id], clearances)
    ...
    def assign_liaison_permissions(self, clearances: list[dict]) -> dict:
        ...
        """
        liaison_permissions_collection = get_clearance_collection("liaison-clearance-permissions")
        record = liaison_permissions_collection.find_one({"campus_id": self.campus_id})
        if record is not None:
            ...
            liaison_permissions_collection.update_one(
                {"campus_id": self.campus_id}, {"$set": {"clearances": allowed_clearances}}
            )
        else:
            record = {"campus_id": self.campus_id, "email": self.email, "clearances": clearances}
    ...
    def revoke_liaison_permissions(self, clearance_guids: list[str]) -> dict:
        ...
        if record is not None:
            ...
            record = {"campus_id": self.campus_id, "email": self.email, "clearances": []}
            liaison_permissions_collection.insert_one(record)
    ...
    def get_liaison_permissions(self) -> list["Clearance"]:
        ...
        return [
            Clearance(
                clearance.get("guid"), clearance.get("id"), clearance.get("name")  # the ccure_id.
            )
