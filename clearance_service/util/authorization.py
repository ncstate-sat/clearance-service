...
from clearance_service.util.authorization_roles import SAT_INTERNAL_ROLES

logger = SATLogger(__name__)


def get_authorization(authorization: str = Header(default=None)) -> TokenPayload:
    ...
    account = TokenPayload(**payload_account)
    logger.debug(f"Authorization payload: {account.model_dump()}")
    return account


def user_is_admin(roles: list) -> bool:
    """Whether the authenticated user has an admin role"""
    return any(user_role in SAT_INTERNAL_ROLES for user_role in roles)
