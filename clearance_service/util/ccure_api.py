...
class CcureApi:
    ...
    def logout(cls):
        ...
        response = requests.post(
            cls.base_url + logout_route, headers={"session-id": cls.get_session_id()}, timeout=1
        )
        cls.session_id = None
    ...
    def get_campus_id_by_email(cls, email) -> str:
        ...
        request_json = {"TypeFullName": "Personnel", "WhereClause": f"Text14 = '{email}'"}
        response = requests.post(
            url,
            json=request_json,
            headers={
    ...
    def get_person_object_id(cls, campus_id: str) -> int:
        ...
        route = "/victorwebservice/api/Objects/FindObjsWithCriteriaFilter"
        url = cls.base_url + route
        request_json = {"TypeFullName": "Personnel", "WhereClause": f"Text1 = '{campus_id}'"}
        response = requests.post(
            url,
            json=request_json,
            headers={
    ...
    def search_people(cls, search: str) -> list[dict]:
        ...
        term_queries = [
            (f"(Text1 LIKE '%{term}%' OR " f"Text14 LIKE '%{term}%')")  # campus_id  # email
            for term in search_terms
        ]
        request_json = {"TypeFullName": "Personnel", "WhereClause": " AND ".join(term_queries)}
        response = requests.post(
            url,
            json=request_json,
            headers={
    ...
    def get_assigned_clearances(cls, assignee_id: int) -> int:
        ...
        request_json = {
            "TypeFullName": (
                "SoftwareHouse.NextGen.Common.SecurityObjects." "PersonnelClearancePairTimed"
            ),
    ...
    def get_clearance_data(cls, clearance_guids: set[str]) -> dict:
        ...
        request_json = {
            "partitionList": [],
            "whereClause": " OR ".join(
                f"GUID = '{clearance_guid}'" for clearance_guid in clearance_guids
            ),
            "pageSize": 0,
            "pageNumber": 1,
            "sortColumnName": "",
        ...
        if response.status_code == status.HTTP_200_OK and response.json():
            return {
                clearance["GUID"]: {"id": clearance["ObjectID"], "name": clearance["Name"]}
        ...
        for assignee, clearance_ids in revocations.items():
            ...
            response = requests.post(
                ...
                json={
                    ...
                    "WhereClause": (f"PersonnelID = {assignee} " f"AND ({clearance_query})"),
                },
                headers={
                    "session-id": cls.get_session_id(),
                    "Access-Control-Expose-Headers": "session-id",
