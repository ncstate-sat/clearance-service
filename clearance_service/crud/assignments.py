...
def get_assignments(
    response: Response, campus_id: str, jwt_payload: dict = Depends(get_authorization)
) -> dict:
    """
...
def assign_clearances(
    ...
    jwt_payload: dict = Depends(get_authorization),
) -> dict:
    """
    ...
    if user_is_admin(jwt_payload):
        ...
        if len(assign_ids) != len(body.clearance_ids):
            ...
            return {"changes": 0, "detail": "Not authorized to assign all selected clearances"}

    try:
        assignment_count = ClearanceAssignment.assign(assigner_email, body.assignees, assign_ids)
    except KeyError:
        response.status_code = status.HTTP_400_BAD_REQUEST
...
def revoke_clearances(
    ...
    revoke_count = ClearanceAssignment.revoke(assigner_email, body.assignees, revoke_ids)
