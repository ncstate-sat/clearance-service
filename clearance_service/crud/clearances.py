...
def get_clearances(
    response: Response, search: Optional[str] = None, jwt_payload: dict = Depends(get_authorization)
) -> dict:
    """
    ...
    if user_is_admin(jwt_payload):
        try:
            ...
            print("CCure timeout. " f"Could not get clearances with search {search}")
