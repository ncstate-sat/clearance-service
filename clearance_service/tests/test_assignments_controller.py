...
from clearance_service.models.clearance_assignment import ClearanceAssignment
...
clearances = [
    {"id": "DECBB54E-4B22-4671-9FA7-F8F370D66A97", "name": "Hunt - Turnstiles"},
    {"id": "E00D2258-4449-4ACD-B640-8163A4D6CAA2", "name": "Library - Faculty Commons"},
    {"id": "75A1AE65-798B-49DA-BDAC-671732AB4794", "name": "Library - Grad Commons"},
    {"id": "57685A64-A1DD-4F7D-8827-7D131F80B38D", "name": "VRB-SAT-Module 1 Contractor Access-C2"},
    {
...
assigned_clearances = [
    {"id": "DECBB54E-4B22-4671-9FA7-F8F370D66A97", "name": "Hunt - Turnstiles", "can_revoke": True},
    {
...
def mock_get_assignments_by_assignee(*_, **__):
    ...
    return [
        ClearanceAssignment(clearance_id="DECBB54E-4B22-4671-9FA7-F8F370D66A97"),
        ClearanceAssignment(clearance_id="E00D2258-4449-4ACD-B640-8163A4D6CAA2"),
    ]
...
def mock_assign(_, assignee_ids: list[str], clearance_ids: list[str]):
    """Mock assigning a clearance"""
    return len(assignee_ids) * len(clearance_ids)


def mock_revoke(_, assignee_ids: list[str], clearance_ids: list[str]):
...
def mock_get_allowed(*_, **__):
    ...
    return [
        Clearance("DECBB54E-4B22-4671-9FA7-F8F370D66A97", name="Clearance1"),
        Clearance("75A1AE65-798B-49DA-BDAC-671732AB4794", name="Clearance2"),
    ]
...
def test_get_assignments_as_admin(monkeypatch):
    ...
    monkeypatch.setattr(
        ClearanceAssignment, "get_assignments_by_assignee", mock_get_assignments_by_assignee
    )

    response = client.get("/assignments/200103374", headers={"Authorization": "Bearer token"})
...
def test_assign_clearances_as_admin(monkeypatch):
    ...
    def mock_request_post(*_, **__):
        ...
        response._content = json.dumps(
            {"data": {"successful": raw_assignees, "failed": []}}
        ).encode("utf-8")
        return response
    ...
    raw_assignees = ["200103374", "200103375"]
    raw_clearance_ids = [
    ...
    response = client.post(
        ...
        json={"assignees": raw_assignees, "clearance_ids": raw_clearance_ids},
    )
    assert response.status_code == 200
