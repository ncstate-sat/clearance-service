...
def mock_get_by_guids(*_, **__):
    ...
    return [{"guid": "D6A233C5-7339-4461-A2DC-89BADD182F97", "id": 5000, "name": "Mock clearance"}]
...
def test_assign_liaison_permissions(monkeypatch):
    ...
    response1 = client.post(
        "/liaison/assign",
        json={"campus_id": "000101234", "clearance_ids": ["D6A233C5-7339-4461-A2DC-89BADD182F97"]},
...
def test_revoke_liaison_permissions(monkeypatch):
    ...
    expected_json = {
        "record": {"campus_id": "000101234", "clearances": [], "email": "test@email.com"}
    }
    assert response1.status_code == 200
    assert response1.json() == expected_json
    assert response2.status_code == 200
...
def test_get_liaison_permissions_without_data(monkeypatch):
    ...
    response = client.get("/liaison?campus_id=000101234", headers={"Authorization": "Bearer token"})
...
def test_get_liaison_permissions_with_data(monkeypatch):
    ...
    get_response = client.get(
        "/liaison?campus_id=000101234", headers={"Authorization": "Bearer token"}
    )
