...
The user documentation (found at https://ncstate-sat.github.io/clearance-service/) is generated by the files in the /docs folder of this repo. However the repository that hosts the site is actually one of our public ones at https://github.com/ncstate-sat/clearance-service. Once the /docs folder has been updated with some new information for users, those changes need to be merged into the main branch. Once merged, checkout the main branch and run the command

```shell
    $> mkdocs gh-deploy
```

This will generate the structure for a static site on the `gh-pages` branch, with a commit msg of `Deployed _someHash_ with MkDocs version: _someVersion_`, where _someHash_ is the abbreviated commit hash of the your merged changes and _someVersion_ is the MkDocs version used in the command.

Since the user guide is hosted on https://github.com/ncstate-sat/clearance-service, we need to then checkout the `gh-pages` branch there, and completely replace everything in it with the static site structure generated from the previous command.

Commit and push those changes to the `gh-pages` branch, and re-use the previous commit msg. Since this repo is hooked up to the user guide Github Pages site, that push will automatically kick off a Github Actions workflow called `pages build and deployment`, which will build and deploy your updated site.
